...
set -euo pipefail
VMID="${1:-50064}"
BIN=target/release/ai-video-autocut
...
echo "=== 배포 → LXC $VMID:/usr/local/bin/ai-video-autocut ==="
pct push "$VMID" "$BIN" /usr/local/bin/ai-video-autocut
pct exec "$VMID" -- chmod +x /usr/local/bin/ai-video-autocut
pct exec "$VMID" -- ls -la /usr/local/bin/ai-video-autocut
